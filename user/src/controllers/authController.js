...
const registerUser = async (req, res) => {
    ...
    if(!username || !email || !name || !password) {
        return res.status(400).send({ message: ResponseMessage.ALL_FIELDS_ARE_REQUIRED });
    }
    try {
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (!isPasswordMatch) {
            return res.status(401).json({ message: ResponseMessage.INVALID_PASSWORD });
        }
        const token = generateToken({ id: user._id, email: user.email, role: user.role });
        const refreshToken = generateRefreshToken(user._id);
        await User.findByIdAndUpdate(user._id, { refreshToken, refreshTokenExp: getRefreshTokenExpiration() })
...
const refreshToken = async(req,res) => {
    ...
    try {
        ...
        if (!user || user.refreshToken !== refreshToken || user.refreshTokenExp < new Date()) {
